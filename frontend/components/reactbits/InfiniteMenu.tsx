...
class InfiniteGridMenu {
  ...
  private initTexture(): void {
    ...
    Promise.all(
      this.items.map(
        item => {
          // handle the case when the image might be undefined
          const src = item.image || item.icon ? `data:image/svg+xml;base64,...` : 'https://picsum.photos/512';
          return new Promise<HTMLImageElement>(resolve => {
            ...
            img.onerror = () => {
              ...
              fallback.src = 'https://picsum.photos/512';
            }
...
const InfiniteMenu: FC<InfiniteMenuProps> = ({ items = [], scale = 1.0, autoRotate = false }) => {
  ...
  useEffect(() => {
    ...
    if (canvas) {
      sketch = new InfiniteGridMenu(
        ...
        sk => {
          sk.run();
          // Automatically rotate slowly if autoRotate is true
          if (autoRotate) {
            // To achieve auto-rotation within the arcball control constraints,
            // we'll apply a gentle continuous rotation velocity if it's completely stopped.
            setInterval(() => {
              const control = (sk as any).control;
              if (control && !control.isPointerDown && Math.abs(control.rotationVelocity) < 0.05) {
                control.rotationVelocity += 0.05;
              }
            }, 500);
          }
        },
  ...
  }, [items, scale, autoRotate]);

  const handleButtonClick = () => {
    const link = activeItem?.href || activeItem?.link;
    if (!link) return;
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      {activeItem && (
        <>
          <h2 className={`face-title ${isMoving ? 'inactive' : 'active'}`}>{activeItem.label || activeItem.title}</h2>

          <p className={`face-description ${isMoving ? 'inactive' : 'active'}`}>{activeItem.description}</p>

          <div onClick={handleButtonClick} className={`action-button ${isMoving ? 'inactive' : 'active'}`} style={{ background: activeItem.color || '#5227ff' }}>
            {activeItem.icon ? (
              <div className="action-button-icon" style={{ top: 0 }}>{activeItem.icon}</div>
            ) : (
